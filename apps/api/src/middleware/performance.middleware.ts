...
class PerformanceMonitor {
  ...
  getMetricsByEndpoint(): Record<string, { count: number; avgDuration: number }> {
    const endpointMetrics: Record<string, { count: number; totalDuration: number }> = {}
...
export const performanceHeadersMiddleware = () => {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    reply.addHook('onResponse', (request, reply, done) => {
      ...
      const duration = Number(endTime - startTime) / 1000000 // Converter para milissegundos
      
      reply.header('X-Response-Time', `${duration.toFixed(2)}ms`)
      reply.header('X-Request-ID', (request as any).requestId || generateRequestId())
...
export const memoryUsageMiddleware = () => {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    reply.addHook('onResponse', (request, reply, done) => {
      ...
      if (memoryDiff.heapUsed > 10 * 1024 * 1024) { // Mais de 10MB
        request.log.warn({
