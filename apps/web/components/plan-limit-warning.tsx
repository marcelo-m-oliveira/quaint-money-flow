...
export function PlanLimitWarning({
  ...
  return (
    <Alert
      className={`${className} ${isAtLimit ? 'border-red-500 bg-red-50' : 'border-yellow-500 bg-yellow-50'}`}
    >
      <AlertTriangle
      ...
      <AlertDescription className="flex items-center justify-between">
        <div className="flex-1">
          <div className="mb-1 flex items-center gap-2">
            <span
              className={`font-medium ${isAtLimit ? 'text-red-800' : 'text-yellow-800'}`}
            >
          ...
          <p
            className={`text-sm ${isAtLimit ? 'text-red-700' : 'text-yellow-700'}`}
          >
            {isAtLimit ? (
              <>
                Você atingiu o limite de <strong>{max}</strong>{' '}
                {max === 1 ? resourceLabelSingular : resourceLabel}
                do seu plano. Para criar mais {resourceLabel}, faça upgrade para
                um plano pago.
              </>
            ) : (
              <>
                Você tem <strong>{remaining}</strong>{' '}
                {remaining === 1 ? resourceLabelSingular : resourceLabel}
